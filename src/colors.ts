...
// Extracted from Twitch Brand Assets/Colors/twitch_color-palette_hex.ai. Grouped by the
// source file's own COLUMNS (not rows) - reading each column top-to-bottom turns out to
// already be a clean set of 8 hue families of exactly 4, light-to-dark. That's a much
// better grouping than computing hue from scratch (which doesn't split evenly), so this
// uses the original designer's own structure.
